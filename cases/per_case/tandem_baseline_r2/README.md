# 基准 case：独立 MATLAB 数据处理目录

源码已分批完成 P1/P2、P3、P4、P5 的本轮改动。**当前只有静态检查通过；MATLAB、Run Section、数值及正式图组验收均未执行。**

1. 在 MATLAB 编辑器打开 `tandem_baseline_r2_case.m`，先只 Run Section 0。它只定义参数/本地路径并检查工作区归属，不读取全部速度、扫描 DAT、统计或绘图。重跑参数节保留已算结果，过期结果在使用时明确报错。
...
默认模式保持：cache/statistics/mean_bl=reuse，phase=skip，structures=compute，transport=skip，最终figures=skip，6–8均skip。不要为试参数 Run 整个脚本，以免按原默认启动正式S4。
