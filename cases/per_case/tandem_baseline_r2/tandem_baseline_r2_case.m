...
% 串列基准工况精简版 r2 主脚本
% 精简的单工况主脚本；在这里直接展示数据处理顺序。
% 本 case 自带 lib/+tblR2、lib/+d23 及 third_party；不借用其他 case。
...
if isempty(case_root) || ~strcmp(script_name, 'tandem_baseline_r2_case')
...
cfg.case_id = 'per_case/tandem_baseline_r2';                          % 工况唯一标识，写入所有阶段元数据。
cfg.case_type = 'baseline';                                           % 工况类型：基准工况不做人工相位分解。
cfg.name = '串列基准工况精简版 r2';                                   % 命令窗口和摘要中显示的工况名称。
...
% 各阶段统一取 'compute'（重新计算）、'reuse'（读取已有缓存）或 'skip'（跳过）。
...
cfg.stages.phase       = 'skip';        % Section 3：baseline 不做相位分解。
cfg.stages.structures  = 'compute';     % Section 4：POD-E50 瞬时结构识别。
...
cfg.stages.figures     = 'skip';        % Section 9：Section 4 图像独立导出。
...
cfg.sources.raw.roots = { ...
    fullfile(input_root, 'Tandem_Baseline_PIV_3rd'), ...
    fullfile(input_root, 'Tandem_Baseline_PIV_2nd')};
cfg.sources.raw.offsets = [0 0];
cfg.sources.raw.ids = {'Tandem_Baseline_PIV_3rd', 'Tandem_Baseline_PIV_2nd'};
cfg.sources.postproc = struct();
cfg.sources.postproc.roots = { ...
    fullfile(input_root, 'Tandem_Baseline_PostProc_3rd'), ...
    fullfile(input_root, 'Tandem_Baseline_PostProc_2nd')};
cfg.sources.postproc.offsets = [0 0];
cfg.sources.postproc.ids = {'Tandem_Baseline_PostProc_3rd', 'Tandem_Baseline_PostProc_2nd'};
...
% E2. 代表性速度剖面提取
cfg.profile = struct(); % 速度剖面配置容器；mode 可选 'fixed'、'multi_avg' 或 'range_avg'，决定剖面的取法。
...
cfg.loglaw.rmse_yplus_range = [100 400]; % 对数律 RMSE 评估的 y+ 区间 [下限 上限]，无量纲且需递增。
...
cfg.loglaw.params.dy_h = 1.0; % 首个保留法向点距壁面的网格间距倍数 dy/h（非负数），h 单位 mm。
...
% 下列字段共同定义传统对数律的适用性筛选：阈值应为有限非负数，点数应为正整数。
cfg.loglaw.applicability.min_y_plus = 30;                 % 最小 y+（无量纲），低于此值不视为对数层。
cfg.loglaw.applicability.max_y_over_delta = 0.15;         % 最大 y/δ（无量纲），限定拟合区不进入外层。
...
cfg.loglaw.applicability.min_contiguous_points = 5;       % 适用区至少包含的连续法向网格点数。
...
cfg.modern_clauser.enabled = true;                         % 是否启用 Option B（逻辑值 true/false）。
cfg.modern_clauser.method = 'rodriguez_lopez_2015_constrained_bump1'; % 唯一支持的复合剖面方法标识。
cfg.modern_clauser.kappa = 0.384;                          % von Kármán 常数 κ（无量纲）。
cfg.modern_clauser.B = 4.17;                               % 对数律截距 B（无量纲）。
cfg.modern_clauser.dy_h = 1.0; % Option B 首个保留法向点距壁面的网格间距倍数 dy/h（非负数）。
cfg.modern_clauser.skip_nearwall = 1;                      % 跳过的近壁网格点数（非负整数）。
...
cfg.modern_clauser.pi_initial = 0.20;                      % 尾迹强度参数 Π 的优化初值（须落在 pi_bounds 内）。
cfg.modern_clauser.pi_bounds = [0 2];                      % Π 的允许范围 [下限 上限]，下限必须非负。
cfg.modern_clauser.u_tau_scale_bounds = [0.5 1.8];        % uτ 缩放因子的允许范围 [下限 上限]。
cfg.modern_clauser.delta_scale_bounds = [0.5 2.0];        % δ 缩放因子的允许范围 [下限 上限]。
cfg.modern_clauser.minimum_points = 10;                    % 拟合所需的最少有效剖面点数（正整数）。
cfg.modern_clauser.max_iterations = 400;                   % 非线性优化的最大迭代次数（正整数）。
cfg.modern_clauser.max_function_evaluations = 1600;        % 目标函数最大评估次数（正整数）。
cfg.modern_clauser.tolerance = 1e-7;                       % 优化收敛容差（正数，无量纲）。
...
cfg.phase = struct('enabled', false);     % baseline 不进行相位平均。
cfg.phase.f0_hz = []; % 外部激励基频，单位 Hz；baseline 的 phase 已关闭，因此用 [] 表示不适用。
cfg.phase.n_bins = []; % 一个周期的相位分箱数（正整数）；baseline 未启用相位分析时留空。
cfg.phase.phi0_user_deg = []; % 用户指定机械零相位，单位 deg；[] 表示未标定并采用程序默认相位。
cfg.phase.minimum_samples_per_bin = []; % 每个相位箱要求的最少样本数（正整数）；phase 关闭时留空。

% -------------------- Section 3 预览图绘制区域配置（用户可改） --------------------
% 剖面图沿流向平均的区域。coherent 分量在激励器附近最强，x 范围过大会将其稀释。
% 默认取整个 FOV 下游段，用户可收紧到激励器影响区（如 [80 180]）以突出 coherent。
% 注：baseline 的 phase 阶段为 skip，此配置在 baseline 中实际不生效，仅保持架构对称。
...
cfg.structures.section4.spatial_exclusion = struct( ...
    'enabled', false, 'x_start_mm', [], 'x_end_mm', [], ...
    'wall_y_height_mm', []);
...
% 保留旧结构配置名称以便旧配置卡/下游读取，但不再由 Section 4 识别器使用。
...
cfg.pod.max_frames = cfg.total_frames; % POD 最多读取的帧数（正整数）；当前使用全部拼接帧。
...
cfg.figures.colormaps.default = 'ocean'; % 未单独指定物理量时的默认色图；可选 turbo、ocean、coolwarm、balance、parula、gray 等项目支持名称。
...
if ~exist('cfg', 'var') || ~exist('paths', 'var') || ~exist('results', 'var') || ...
        ~isfield(cfg, 'case_id') || ~strcmp(cfg.case_id, 'per_case/tandem_baseline_r2') || ...
...
if logical(cfg.preview.enabled) && ~isempty(results.phase) && ...
    ...
    phase_contour = results.phase; % baseline 保留原相位输入。
    tblR2.require_case_library(fileparts(cfg.script_file), ...
