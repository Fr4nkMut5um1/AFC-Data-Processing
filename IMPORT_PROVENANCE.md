...
原始分批 Git 历史保留在已交付的 PIV_r2_current_code.zip 内 version_history.bundle。
本次自动审批拒绝将完整历史归档加入 GitHub，因此主分支仅导入当前代码快照和观察层。
原包文件清单保存在 `source-history/release_manifest.json`。
历史恢复请使用原交付包中的 bundle；本仓库不声称具有原 c860e5d 的原生提交链。
